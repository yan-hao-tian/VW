...
class BasePixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.linear_fuse = Conv2d(1536, 512, kernel_size=1, bias=use_bias, norm=output_norm, activation=F.relu,)
        ...
        self.attn = nn.Sequential(*[nn.MultiheadAttention(512, 8, batch_first=True) for _ in [2, 4, 8]]) 
        self.ds = nn.ModuleList([Conv2d(512, 512//(k**2), kernel_size=k, padding=0,norm=get_norm('SyncBN', 512//(k**2)), activation=F.relu,) 
                                    for k in [2, 4, 8]])        
        
        output_norm_ = get_norm(norm, 256)
        self.short_path_ = Conv2d(256, 256, kernel_size=1, bias=use_bias, norm=output_norm_, activation=F.relu,)      
        # self.image_pool = nn.Sequential(nn.AdaptiveAvgPool2d(1), 
                            # Conv2d(512, 512, kernel_size=1, bias=use_bias, norm=output_norm, activation=F.relu,))
        self.cat_ = Conv2d(1280, 256, kernel_size=1, bias=use_bias, norm=output_norm_, activation=F.relu,)
        self.attn_ = nn.Sequential(*[nn.MultiheadAttention(256, 4, batch_first=True) for _ in [2, 4, 8, 16]]) 
        self.ds_ = nn.ModuleList([Conv2d(256, 256//(k**2), kernel_size=k, padding=0,norm=get_norm('SyncBN', 256//(k**2)), activation=F.relu,) 
                                    for k in [2, 4, 8, 16]])    
    ...
    def forward_features(self, features):
        ...
        # print(c4.shape, c3.shape, c2.shape, c1.shape)
        n, _, h, w = c4.shape
        ...
        _c = output
        output = []
        output.append(self.short_path_(_c))
        # output.append(F.interpolate(
        #         self.image_pool(_c),
        #         size=_c.size()[2:],
        #         mode='nearest'))
        ...
        query = rearrange(_c, 'b c (nh ph) (nw pw) -> (b nh nw) (ph pw) c', nh=nh, nw=nw)
        # print(query.shape)
        for j, r in enumerate([2, 4, 8, 16]):
            rh = rw = r
            pad_w = [rw//2-1, rw//2] if rw%2 ==0 else [rw//2]*4
            pad_h = [rh//2-1, rh//2] if rh%2 ==0 else [rh//2]*4
            context = F.pad(_c, pad_w+pad_h)
            context = self.ds_[j](context)
            context = torch.cat([context[...,(rw+1)*pw//2:rw*pw],
                                context,
                                context[...,-(rw*pw):-((rw+1)*pw//2)]
                                ], dim=3)  
            ...
            context = rearrange(context, 'b (c ph pw) (nh nw) -> (b nh nw) c ph pw', 
                                ph=ph*rh, pw=pw*rw, nh=nh, nw=nw)
            context = rearrange(context, 'b c (ph nh) (pw nw) -> b (ph pw) (nh nw c)', nh=rh, nw=rw)
            _output, _ = self.attn_[j](query, context, context)
            _output = rearrange(_output, '(b nh nw) (ph pw) c -> b c (nh ph) (nw pw)', nh=nh, nw=nw, ph=ph, pw=pw)
            output.append(_output)  
        output = self.cat_(torch.cat(output, dim=1))
