...
def heatmap(data, camp='Spectral', figsize=(10, 10.75), ax=None, save_path=None):
    ...
    ax = sns.heatmap(data,
                ...
                # vmin=0,
                # vmax=1,
                center=0, robust=True, annot=False, ax=ax, cbar=True, annot_kws={"size": 24}, fmt='.2f')
...
def single_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        # warnings.warn(data.keys())
        # print(data['img'][0].shape)
        # with torch.no_grad():
        # if meter.count == 50:
        #     np.save(out_dir, meter.avg)
        #     exit()
        # print(data[img].keys())
        ori_file_name = data['img_metas'][0].data[0][0]['ori_filename']
        chosen_file = 'ADE_val_00000572.jpg'
        if ori_file_name == chosen_file:
        # print(data['img_metas'][0].data[0][0]['ori_filename'])
        # print('ishere')
            data['img'][0].requires_grad = True
            optimizer.zero_grad()    
            result = model(return_loss=False, **data)
            outputs = result[0].unsqueeze(0)
            # outputs = rearrange(result[0], 'b (h w) c -> b c h w', h=14, w=14)
            out_size = outputs.size()
            
            central_point = torch.nn.functional.relu(outputs[:, :, int(out_size[2] * 0.95), int(out_size[3] * 0.25)]).sum()
            grad = torch.autograd.grad(central_point, data['img'][0])
            grad = grad[0]
            grad = torch.nn.functional.relu(grad)
            aggregated = grad.sum((0, 1))
            grad_map = aggregated.cpu().numpy()
            # print(grad_map.shape)
            ori_file_name = ori_file_name.replace('.jpg', '.npy')
            np.save(f'{out_dir}/{ori_file_name}', grad_map)
            # data = np.load()
            
            data = np.log10(grad_map + 1)       #   the scores differ in magnitude. take the logarithm for better readability
            # data = (data - np.min(data)) / (np.max(data) - np.min(data))      #   rescale to [0,1] for the comparability among models
            # print(np.max(data))
            # print(np.min(data))
            ...
            save_path_heatmap = f'{out_dir}/{ori_file_name}'.replace('.npy', '.png')
            heatmap(data, save_path=save_path_heatmap)
            ...
            if np.isnan(np.sum(grad_map)):
                ...
                try:
                    ...
                    print('Not accumulated because size error')
